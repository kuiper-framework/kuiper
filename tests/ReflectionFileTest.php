<?php
namespace kuiper\reflection;

use kuiper\test\TestCase;

class ReflectionFileTest extends TestCase
{
    public function testGetClasses()
    {
        $parser = new ReflectionFile(__DIR__.'/fixtures/DummyClass.php');
        $this->assertEquals(
            $parser->getClasses(),
            ['kuiper\\reflection\\fixtures\\DummyClass']
        );
    }

    public function testGetClassesMultiple()
    {
        $parser = new ReflectionFile(__DIR__.'/fixtures/MultipleNamespaces.php');
        $this->assertEquals(
            $parser->getClasses(),
            [
                0 => 'NamespaceA\\ClassA',
                1 => 'NamespaceA\\InterfaceA',
                2 => 'NamespaceB\\ClassB',
                3 => 'NamespaceB\\InterfaceB',
                4 => 'ClassC',
                5 => 'InterfaceC',
            ]
        );
    }

    public function testGetImports()
    {
        $parser = new ReflectionFile(__DIR__.'/fixtures/TestUse.php');
        // var_export($parser->getImports());
        $this->assertEquals(
            $parser->getImports(),
            ["foo" => [
                'classes' => [
                    'Another' => 'My\\Full\\Classname',
                    'NSname' => 'My\\Full\\NSname',
                    'ArrayObject' => 'ArrayObject',
                    'AnotherB' => 'My\\Full\\ClassnameB',
                    'NSnameB' => 'My\\Full\\NSnameB',
                    'ClassA' => 'some\\ns\\ClassA',
                    'ClassB' => 'some\\ns\\ClassB',
                    'C' => 'some\\ns\\ClassC',
                ],
                'functions' => [
                    'functionName' => 'My\\Full\\functionName',
                    'func' => 'My\\Full\\functionName',
                    'fn_a' => 'some\\ns\\fn_a',
                    'fn_b' => 'some\\ns\\fn_b',
                    'fn_c' => 'some\\ns\\fn_c',
                ],
                'constants' => [
                    'CONSTANT' => 'My\\Full\\CONSTANT',
                    'ConstA' => 'some\\ns\\ConstA',
                    'ConstB' => 'some\\ns\\ConstB',
                    'ConstC' => 'some\\ns\\ConstC',
                ],
            ]]
        );
    }

    public function testGetImports70()
    {
        $parser = new ReflectionFile(__DIR__.'/fixtures/TestUse70.php');
        $this->assertEquals(
            $parser->getImports(),
            [''=>[
                'classes' => [
                    'ClassA' => 'some\\ns\\ClassA',
                    'C' => 'some\\ns\\ClassC',
                ]
            ]]
        );
    }

    public function testGetImportsMultiple()
    {
        $parser = new ReflectionFile(__DIR__.'/fixtures/TestUseMulitipleNamespace.php');
        // var_export($parser->getImports());
        $this->assertEquals(
            $parser->getImports(),
            [
                'foo' => [
                    'classes' => [
                        'Another' => 'My\\Full\\Classname',
                    ]
                ],
                'bar' => [
                    'classes' => [
                        'Classname' => 'My\\Full\\Classname',
                    ]
                ],
            ]
        );
    }
}